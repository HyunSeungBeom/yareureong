...
- **리그가 다르면 절대 섞지 않는다.** `team`·`game` 에 `league` 컬럼이 있고(`League` enum + `LeagueConverter`,
...
- **패키지는 같은 모양으로 자란다** — 컨트롤러(얇게) · 서비스 · DTO 파일 · 순수 계산.
  컨트롤러 파일에 DTO·보조 서비스를 얹지 않고, 컨트롤러는 리포지토리를 직접 받지 않는다.
  `PackageConventionTest` 가 KNOWN 목록과 대조한다(예외는 **이유와 함께** 올린다)
